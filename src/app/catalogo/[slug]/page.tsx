...
export default function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    fetchPublicProducts().then((items) => {
      ...
      setProduct(current);
      setRelated(current ? items.filter((item) => item.category === current.category && item.id !== current.id).slice(0, 3) : []);
  ...
  return <><SiteHeader /><main className="mx-auto max-w-7xl px-5 py-8 md:py-14"><Link className="inline-flex items-center gap-2 text-sm text-white/60 hover:text-gold" href="/catalogo"><ArrowLeft size={16} /> Volver al catálogo</Link><section className="mt-8 grid gap-10 md:grid-cols-2 md:gap-20"><div className="relative grid aspect-square place-items-center overflow-hidden bg-graphite">{mainImage ? <img src={mainImage.url} alt={mainImage.alt || product.name} className="h-full w-full object-contain object-center" /> : <span className="text-sm text-white/35">Sin imagen</span>}</div><div className="flex flex-col justify-center"><p className="eyebrow">{product.category}</p><h1 className="mt-4 font-display text-5xl md:text-6xl">{product.name}</h1><p className="mt-5 text-2xl">{product.price > 0 ? `$ ${product.price.toLocaleString("es-UY")}` : "Consultar precio"}</p><p className="mt-8 max-w-lg leading-7 text-white/60">{product.description || "Consultanos por WhatsApp para conocer todos los detalles."}</p><dl className="mt-9 border-y border-white/10 py-6 text-sm"><div><dt className="text-white/40">Disponibilidad</dt><dd className="mt-1">{product.stock > 0 ? "Disponible" : "Consultar"}</dd></div></dl><a target="_blank" href={whatsappUrl(product)} className="mt-8 flex w-fit items-center gap-2 rounded-full bg-rose px-7 py-3.5 text-sm font-semibold transition hover:bg-[#c6818b]"><MessageCircle size={17} /> Comprar por WhatsApp</a><p className="mt-5 flex items-center gap-2 text-xs text-white/45"><ShieldCheck size={15} className="text-gold" /> Atención y envíos con total discreción.</p></div></section>{related.length > 0 && <section className="mt-28"><p className="eyebrow">También puede interesarte</p><h2 className="mt-3 font-display text-4xl">Continúa explorando</h2><div className="mt-10 grid grid-cols-2 gap-4 md:grid-cols-3">{related.map((item) => <ProductCard key={item.id} product={item} />)}</div></section>}</main><SiteFooter /></>;
}
