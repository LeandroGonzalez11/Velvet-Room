...
const TAG_KEYWORDS: Record<string, string[]> = {
  juguetes: ["vibrad", "dildo", "estimulad", "plug", "varita", "capsula", "cápsula", "masturbad", "anillo", "huevo"],
  bienestar: ["gel", "lubricante", "ducha", "higien", "afrodis", "excitante", "gotas", "miel", "energiz"],
  bdsm: ["esposas", "mordaza", "collar", "mascara", "máscara", "latigo", "látigo", "separador", "antifaz"],
};
...
export function Catalogue() {
  ...
  const result = useMemo(() => products.filter(product => (category === "Todos" || product.category === category) && product.name.toLowerCase().includes(query.toLowerCase()) && (!keywords.length || keywords.some(k => product.name.toLowerCase().includes(k)))).sort((a, b) => sort === "low" ? a.price - b.price : sort === "high" ? b.price - a.price : sort === "name" ? a.name.localeCompare(b.name) : new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()), [products, query, category, sort, keywords]);
